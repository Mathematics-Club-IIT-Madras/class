...
test_that("GenIBOSS returns subselected data and final_model in matrix mode", {
  ...
  set.seed(901)
  out <- GenIBOSS(X = d$X, y = d$y, nSample = 20, k = 12, family = gaussian(), add_logs = TRUE)
...
test_that("GenIBOSS returns subselected data and final_model in csv mode", {
  ...
  set.seed(902)
  out <- GenIBOSS(csv = path, nSample = 25, k = 16, family = gaussian(), header = TRUE, add_logs = TRUE)
...
test_that("GenIBOSS works across supported families", {
  ...
  for (case in cases) {
    ...
    out <- GenIBOSS(X = d$X, y = case$y, nSample = 30, k = 20, family = case$family, add_logs = TRUE)
